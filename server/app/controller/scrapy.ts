...
let resultArticleDetail: Array<any> = [];
let currentArticleMap: any = {};
...
interface ArticleInfoExtra {
    cid: string
}

interface ArticleInfo {
    extra: ArticleInfoExtra
    id: string
}
...
export default class Article extends Controller {
    ...
    getLocalArticleList() {
        ...
        this.startDownArticleDetail(list);
    }
    ...
    getNetworkArticleList() {
        return axios({
            ...
            currentArticleMap = readDataFromFile('./articles/articleDetailMap.json');
            this.startDownArticleDetail(res.data.data.list);
            console.log(res.data.data.page.count);
        })
    }

    async getComments(info: ArticleItem, score = 0) {
        return axios({
            ...
            return page.more ? list.concat(await this.getComments(info, list[list.length - 1].score)) : list
        });
    }

    async startDownArticleDetail(data: Array<any> = []) {
        while(data.length) {
            const info: ArticleItem = data.shift();
            try {
                if (!currentArticleMap[info.id]) {
                    await this.saveArticleContent(info);
                    await delay(2000);
                }
            } catch (err) {
                data.push(info);
        ...
        this.formatArticleList2Map(fullArticleDetailList);
        const navList = await this.getNavList();
        const articleInfo = await this.getArticleInfo()
        this.mergeArticleData(articleInfo, fullArticleDetailList, navList);
    }

    saveArticleContent(info: ArticleItem) {
        return axios({
            ...
            const { data } = res.data;
            data.commentsList = await this.getComments(info);
    ...
    async formatArticleList2Map(articleList: Array<any> = []) {
        let articleId2Map = {};
        while(articleList.length) {
            let item: ArticleItem = articleList.shift();
            item.noteList = await this.getNoteList(item);
    ...
    mergeArticleData(info: ArticleInfo, articleDetailList, navList) {
